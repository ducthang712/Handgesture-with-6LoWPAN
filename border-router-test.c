...
udp_rx_callback(struct simple_udp_connection *c,
		...
		uint16_t datalen)
{
  printf("Receive data: %.*s from ",datalen, (char *)data);
  uip_debug_ipaddr_print(sender_addr);
  printf("\n");

  // Xử lý dữ liệu nhận được từ gói tin
  if (strncmp((char *)data, "Hello", 5) == 0) {
    printf("Received a Hello message from Python!\n");
  }
}
...
#endif
  // Tạo kết nối UDP lắng nghe trên cổng UDP_PORT
